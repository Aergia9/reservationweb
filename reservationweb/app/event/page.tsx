...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        ...
        toast.error('Error connecting to Firestore: ' + error.message);
      }
    );
  ...
  const handleCreate = async () => {
    if (!formData.name.trim() && !formData.description.trim()) {
      toast.error('Please fill in at least the name or description');
      return;
    }
    if (!user) {
    ...
    try {
      ...
      if (selectedImageFiles.length > 0) {
        try {
          ...
          toast.warning('Failed to upload some images, continuing with uploaded ones.');
        }
      }
  ...
  const handleUpdate = async () => {
    ...
    if (!user) {
      toast.error('You must be authenticated to update items');
      return;
    }
    ...
    try {
      ...
      if (selectedImageFiles.length > 0) {
        try {
          ...
          console.error('Error uploading images:', uploadError);
          toast.warning('Failed to upload some images, keeping existing images.');
        }
      }
